...
module.exports = (app) => {
  ...
  app.post('/send-submission', (req, res) => {
    ...
    Presenter.findOne({ email: req.body.email }, (err, presenter) => {
      ...
      // If no presenter mail has been provided
      if (presenter == null) res.send('Provide a valid presenter email.');

      // Go through the process of creating a submission
      else {
        ...
        Author.create(authors, (err, authors) => {
          ...
          // Once the presenter and authors are created/found, create the new submission with the associated feedback
          ...
          feedback.save((err) => {
            ...
            submission.save(res.redirect('/view-submissions?email=' + presenter.email));
          });
        });
      }
    });
