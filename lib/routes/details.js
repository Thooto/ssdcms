...
module.exports = (app, isLoggedIn) => {
  app.get('/details/:id', isLoggedIn, async (req, res) => {
    ...
    if (submission.authors.length == 1) submission.authors = submission.authors[0].split(',')

    let authorIds = Array.from(submission.authors.filter(id => id != 'not provided yet'), id => ObjectId(id))
